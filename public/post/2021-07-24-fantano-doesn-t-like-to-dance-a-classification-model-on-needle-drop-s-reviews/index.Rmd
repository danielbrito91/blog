...
Anthony Fantano (@theneedledrop, a.k.a Melon) has been reviewing albums on his [YouTube channel](https://www.youtube.com/c/theneedledrop/videos) for more than one decade. I have been enjoying his work and discovering some great music, at least since 2014.

Recently, I was wondering if I could fit a classification model to predict if the reviewed album would get a high score. Besides trying to predict if Fantano would love an album, I wanted to understand what features drive his scoring. And, as the title of this post suggests, I found that the danceability of an album isn't a feature that Melon appreciates.

Using Python and R, I got the audio features of the albums from Spotify using `spotipy`, cleaned the data in R, and fitted and evaluated a predictive model with Scikit-Learn. The whole process is described in this post, which was written in R Markdown and published with [blogdown](https://bookdown.org/yihui/blogdown/).
...
## Audio features
Later, I pulled the albums' audio features in Python with `spotypy`. The code can be accessed here. The following features were gathered, whose description can be found on [Spotify Web API documentation](https://hexdocs.pm/spotify_web_api/api-reference.html):
...
I also get the album's popularity (`album_popularity`, a value between 0 and 100, with 100 being the most popular) and the list of genres associated with the artist (`artist_genres`). The gathered data were stored on the `spotify_songs.csv` file (available on my GitHub).
...
Finally, I selected the first valid value in the features that contained genres and consolidate the data in  `album_review_features`.
...
When just lumping those genres, a lot of information is lost. So, to deal with it, I replaced the specifics subgenres with a more broad view of the genres: instead of "alternative hip hop", "hip hop", "atl hip hop" and the variations that appear on the data, I will call it just "hip hop". The same idea was applied to rock, metal, punk, pop, folk, r&b, electronic, regional music, experimental, jazz and ambient. Some of the subgenres were too specific (afrofuturism, escape room, chamber psych and permanent wave) and contained too many observations, so, they were replaced regarding the artists associated with them. I considered the [Rate Your Music](https://rateyourmusic.com/genres) data as a major reference here.
...
After that, most of the reviewed albums fall into the hip hop genre, followed by rock, pop, metal, electronic, punk and alternative dance. Goodbye wu fam, plunderphonics and meme rap.
...
I didn't consider the albums without numerical scores and the albums that I didn't find on Spotify. So, filtering that data we have a total of 2097 albums to work with to create our predictors, target and labels.
...
Let's go to Python to preprocess the data using Scikit-Learn. First, I imported the data into the Python environment. I did the train/test splitting stratifying by the score values, considering that we have few observations with very low (0-3) and very high (9-10) scores. I used 20% of the data as test data.
...
We can see that precision falls quickly. It is far from the perfect classifier, but it shows some improvement compared to the baseline classifier (all albums being loved). With a threshold of 0.2, we get close to 80% of the loved albums with precision near 30%; with a threshold of 0.5, almost no loved album would be identified.
...
def plot_rec_prec(trained_model, plot_title):
  ...
  _ = plt.xticks(np.arange(0, 1.01, step=0.1))
  _ =  plt.yticks(np.arange(0, 1.01, step=0.1))
  
  _ = plt.subplot(1, 2, 2)
  _ = plt.grid(True)
  _ = plt.plot(thresholds, precisions[:-1], "b--", label = "Precision")
  _ = plt.plot(thresholds, recalls[:-1], "g-", label ="Recall")
...
The Extremely Randomized Trees (or Extra-trees) model makes the Random Forest more random by using random thresholds for each feature. I thought in giving it a try, checking if a higher randomization level could improve the results. I also performed hyperparameter tunning in `min_samples_split`, `min_samples_leaf` and `max_features`.
...
The simplest model to use and interpret in this post, I fitted a logistic regression model using a ridge regularization (setting `penalty = “l2”`). I tunned the inverse of regularization strength (`C`) hyperparameter.
...
The selected model achieved an AUC score of `r round(py$test_roc_auc, 4)`, better than the dummy predictor (0.50),  similar to the value found on the train set with cross-validation. It is a quite limited classifier, but it can help distinguish the loved albums to some extent.
...
pdp_in_r = partial_dependence(etc, X_train_prep, features = [6],
...
I wanted to check how the mean danceability of an album affected the dependence of the prediction for each instance. So, I plotted the individual conditional expectation (ICE) curve. I could not obtain the ICE curve values with `reticulate`, so I got it in the Jupyter Notebook using the same code. The ICE plot shows that mean danceability doesn't markedly influence the predicted values for some albums with a higher probability of being loved. But, although with some noise, the plot reveals a decreasing predicted chance of getting a high score when increasing the mean danceability.
...
Thanks for reading! I'm open to any feedback or constructive criticism. You can reach me on Twitter @daniel_brito_
